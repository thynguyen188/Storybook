...
public class ManagePanel extends AbstractScrollPanel {
	...
	protected int getZoomValue() {
		Internal internal = BookUtil.get(mainFrame,
				BookKey.MANAGE_ZOOM, SbConstants.DEFAULT_MANAGEZOOM);
		return internal.getIntegerValue();
	}

	@Override
	protected int getMinZoomValue() {
		return SbConstants.DEFAULT_MANAGEMINZOOM;
	}

	@Override
	protected int getMaxZoomValue() {
		return SbConstants.DEFAULT_MANAGEMAXZOOM;
	}
	...
	public void modelPropertyChange(PropertyChangeEvent evt) {
		...
		if (BookController.ManageViewProps.HIDE_UNASSIGNED.check(propName)) {
			init();
			refresh();
			return;
		}
	...
	public void init() {
		...
		try {
			Internal internal = BookUtil.get(mainFrame, BookKey.MANAGE_COLUMNS, SbConstants.DEFAULT_MANAGECOLUMNS);
			cols = internal.getIntegerValue();
		} catch (Exception e) {
			System.err.println(Arrays.toString(e.getStackTrace()));
			cols = SbConstants.DEFAULT_MANAGECOLUMNS;
		}
	}
	...
	public void refresh() {
		...
		if (mainFrame.showAllParts) chapters = dao.findAll();
		else chapters = dao.findAll(part);
		model.commit();

		removeAll();

		// "chapter" for unassigned scenes
		if (!BookUtil.getBoolean(mainFrame, BookKey.MANAGE_HIDE_UNASSIGNED)) {
		...
		if (panel.getComponentCount() == 0) {
			panel.add(new JLabel(I18N.getMsg("warning.no.chapters")));
		}
		revalidate();
		repaint();
	}
