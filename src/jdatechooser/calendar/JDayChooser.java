...
public class JDayChooser extends JPanel implements ActionListener, KeyListener,
	...
	private void drawDayNames() {
		...
		int lday = firstDayOfWeek;

		for (int i = 0; i < 7; i++) {
			if (maxDayCharacters > 0 && maxDayCharacters < 5) {
				if (dayNames[lday].length() >= maxDayCharacters) {
					dayNames[lday] = dayNames[lday]
							.substring(0, maxDayCharacters);
				}
			}

			days[i].setText(dayNames[lday]);
	...
	protected void drawDays() {
		...
		Date lday = tmpCalendar.getTime();
		int n = 0;
		Color foregroundColor = getForeground();

		while (lday.before(firstDayInNextMonth)) {
			...
			while (iterator.hasNext()) {
				...
				if (dateEvaluator.isSpecial(lday)) {
				...
				if (dateEvaluator.isInvalid(lday)){
					days[i + n + 7].setForeground(dateEvaluator
	...
	public void actionPerformed(ActionEvent e) {
		...
		int lday = Integer.parseInt(buttonText);
		setDay(lday);
	}
