...
public final class ODTUtils {
	...
	private static Map<Scene, Integer> getScenesSizeOrWords(MainFrame mainFrame, boolean wordsCount) {
		Map<Scene, Integer> sceneSizes = new HashMap<>();
		// Get scenes
		sceneSizes.clear();
		BookModel model = mainFrame.getBookModel();
		Session session = model.beginTransaction();
		...
		for (Scene scene : scenes) {
			if (scene.getChapter() != null) {
				...
				} else if (wordsCount) { // words in summary
					int size = TextUtil.countWords(scene.getSummary());
					sceneSizes.put(scene, size);
				} else {
					sceneSizes.put(scene, (HtmlUtil.htmlToText(scene.getSummary()).length()));
				}
			}
		}
		return sceneSizes;
	...
	public static String getDefaultFilePath(MainFrame mainFrame, Scene scene) {
		...
		if (BookUtil.isUseXeditor(mainFrame)) {
			ext = "." + BookUtil.getXeditorExtension(mainFrame);
		}
		...
		SbApp.trace("Scene external file=" + str);
		return (str);
	}
	 ...
	 *
	 * @param filePath path to the file
	 * @param countWords true to count words instead of characters
	 * @return the size in characters (approximated) or zero.
	 */
	public static int getDocumentSizeOrWords(String filePath, boolean countWords) {
		int ret = 0;
		if (filePath == null || !filePath.endsWith(".odt")) {
			return (0);
		}
		ODPackage pkg;
		try {
	...
	public static File createNewODT(MainFrame mainFrame, String name) {
		//System.out.println("ODTUtils.createNew("+name+")");
		...
		if (JOptionPane.showConfirmDialog(null,
			I18N.getMsg(type + ".filenotexist"),
			I18N.getMsg(type + ".launching"),
			JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
			try {
				String source = "storybook/resources/Empty.odt";
				if (BookUtil.isUseNoTemplate(mainFrame)) {
					source = "storybook/resources/NoTemplate.odt";
				} else if (BookUtil.isUseSimpleTemplate(mainFrame)) {
					source = "storybook/resources/Simple.odt";
				}
				InputStream is;
				if (BookUtil.isUseXeditor(mainFrame)) {
					is = new FileInputStream(new File(BookUtil.getXeditorTemplate(mainFrame)));
				} else {
					is = mainFrame.getClass().getClassLoader().getResourceAsStream(source);
				}
				Files.copy(is, file.toPath());
			} catch (IOException ex) {
				SbApp.error("ODTUtils.createNewODT(" + name + ")", ex);
			}
		} else {
			return (null);
		}
	...
	public static void launchExternalEditor(MainFrame mainFrame,Scene scene) {
		SbApp.trace("ODTUtils.launchExternalEditor("+ "mainFrame," + scene.getFullTitle() +")");
		String name;
		String type = "libreoffice";
		if (BookUtil.isUseXeditor(mainFrame)) {
			type = "xeditor";
