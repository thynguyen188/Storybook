...
public class SbImageDialog extends AbstractDialog {
	...
	public SbImageDialog(MainFrame m, JComponent parent) {
		super(parent);
		mainFrame = m;
		initAll();
	}
	...
	public void initUi() {
		...
		setTitle(I18N.getMsg("editor.insert.image"));

		JLabel lbFilename = new JLabel(
			I18N.getColonMsg("file.info.filename"));
		...
		JLabel lbWidth = new JLabel(I18N.getColonMsg("width"));
		...
		JLabel lbHeight = new JLabel(I18N.getColonMsg("height"));
	...
	private class ChooseImageFileAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (mainFrame != null) {
				String dir = mainFrame.getLastImageDir();
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				try {
					...
					String n = file.toURI().toURL().toString();
					if (!n.startsWith("file://")) n=n.replace("file:", "file://");
					tfFilename.setText(n);
					...
					mainFrame.setLastImageDir(file);
				} catch (Exception e1) {
				}
			}
		}
