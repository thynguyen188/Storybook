...
public class SceneTable extends AbstractTable {
	...
	private JComboBox sceneNarratorCombo;
	...
	public List<AbstractEntity> getAllEntities() {
		...
		if (sceneNarratorCombo.getSelectedIndex()!=0) {
			String selNarrator=(String)sceneNarratorCombo.getSelectedItem();
			Iterator iScene=ret.iterator();
			Scene scene;
			while (iScene.hasNext()) {
				scene=(Scene)iScene.next();
				if ((sceneNarratorCombo.getSelectedIndex()!=0)
						&&(!scene.getNarrator().getFullName().equals(selNarrator))) iScene.remove();
			}
		}
		model.commit();
	...
	protected void initOptionsPanel() {
		SbApp.trace("SceneTable.initOptionsPanel()");
		optionsPanel.removeAll();
		JLabel lbState = new JLabel(I18N.getColonMsg("status"));
		...
		JLabel lbStrand = new JLabel(I18N.getColonMsg("strand"));
		...
		sceneStrandCombo.addItem(I18N.getMsg("status.all"));
		...
		optionsPanel.add(new JLabel(I18N.getColonMsg("scene.narrator")));
		sceneNarratorCombo = new JComboBox();
		sceneNarratorCombo.setName(SbConstants.ComponentName.COMBO_SCENE_NARRATOR.toString());
		PersonDAOImpl personDAO=new PersonDAOImpl(session);
		List<Person> persons=personDAO.findAll();
		sceneNarratorCombo.addItem(I18N.getMsg("status.all"));
		for (Person p : persons) sceneNarratorCombo.addItem(p.getFullName());
		sceneNarratorCombo.setSelectedIndex(0);
		sceneNarratorCombo.addActionListener(this);
		optionsPanel.add(sceneNarratorCombo);
	}
	...
	public synchronized void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() instanceof JComboBox) {
			...
			if (SbConstants.ComponentName.COMBO_SCENE_STATES.toString().equals(cb.getName())) {
				SceneState state = (SceneState) sceneStateCombo.getSelectedItem();
				mainFrame.getBookController().filterScenesStatus(state);
				return;
			}
			...
			if (SbConstants.ComponentName.COMBO_SCENE_NARRATOR.toString().equals(cb.getName())) {
				String person = (String) sceneNarratorCombo.getSelectedItem();
				mainFrame.getBookController().filterScenesNarrator(person);
				return;
			}
		}
...
//			Object oldValue = evt.getOldValue();
			...
			if (BookController.SceneProps.FILTER_STATUS.check(propName)) {
				initTableModel(evt);
			...
			if (BookController.SceneProps.FILTER_NARRATOR.check(propName)) {
				initTableModel(evt);
				if (newValue instanceof String) {
					sceneNarratorCombo.setSelectedItem((String) newValue);
				}
				return;
			}
			if (BookController.SceneProps.FILTER_STRAND.check(propName)) {
				initTableModel(evt);
				if (newValue instanceof String) {
...
//				initTableModel(evt2);
				...
				&& ((SbView)evt.getNewValue()).getName().equals("Scenes")) {
