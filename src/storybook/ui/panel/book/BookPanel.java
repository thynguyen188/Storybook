...
public class BookPanel extends AbstractScrollPanel {
	...
	protected int getZoomValue(){
		Internal internal = BookUtil.get(mainFrame,
				BookKey.BOOK_ZOOM, SbConstants.DEFAULT_BOOKZOOM);
		return internal.getIntegerValue();
	}

	@Override
	protected int getMinZoomValue() {
		return SbConstants.DEFAULT_BOOKMINZOOM;
	}

	@Override
	protected int getMaxZoomValue() {
		return SbConstants.DEFAULT_BOOKMAXZOOM;
	}
	...
	public void refresh() {
		...
		if (mainFrame.showAllParts) chapters = dao.findAllOrderByChapterNoAndSceneNo();
		else chapters = dao.findAllOrderByChapterNoAndSceneNo(part);
		model.commit();
