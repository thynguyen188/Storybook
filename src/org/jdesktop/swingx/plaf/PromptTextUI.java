...
/**
 * <p>
 * Abstract {@link TextUI} class that delegates most work to another {@link TextUI} and additionally renders a prompt
 * text as specified in the {@link JTextComponent}s client properties by {@link PromptSupport}.
 * <p>
...
public abstract class PromptTextUI extends TextUI {
	...
	/**
	 * Calls TextUI#installUI(JComponent) on the delegate and installs a focus listener on <code>c</code> which repaints
	 * the component when it gains or loses the focus.
	 */
	@Override
	...
	/**
	 * Creates a label component, if none has already been created. Sets the prompt components properties to reflect the
	 * given {@link JTextComponent}s properties and returns it.
	 *
	 * @param txt
	...
	/**
	 * When {@link #shouldPaintPrompt(JTextComponent)} returns true, the prompt component is retrieved by calling
	 * {@link #getPromptComponent(JTextComponent)} and it's preferred size is returned. Otherwise
	 * super{@link #getPreferredSize(JComponent)} is called.
	 */
	@Override
	...
	/**
	 * Delegates painting when {@link #shouldPaintPrompt(JTextComponent)} returns false. Otherwise the prompt component
	 * is retrieved by calling {@link #getPromptComponent(JTextComponent)} and painted. Then the caret of the given text
	 * component is painted.
	 */
	@Override
	...
	/**
	 * Returns if the prompt or the text field should be painted, depending on the state of <code>txt</code>.
	 *
	 * @param txt
	...
	/**
	 * Calls super.{@link #update(Graphics, JComponent)}, which in turn calls the paint method of this object.
	 */
	@Override
	...
	/**
	 * Delegate when {@link #shouldPaintPrompt(JTextComponent)} returns false. Otherwise get the prompt component's UI
	 * and delegate to it. This ensures that the {@link Caret} is painted on the correct position (this is important
	 * when the text is centered, so that the caret will not be painted inside the label text)
	 */
	@Override
	...
	/**
	 * Calls {@link #modelToView(JTextComponent, int, Bias)} with {@link Bias#Forward}.
	 */
	@Override
	...
	/**
	 * Tries to call {@link ComponentUI#getBaseline(int, int)} on the delegate via Reflection. Workaround to maintain
	 * compatibility with Java 5. Ideally we should also override {@link #getBaselineResizeBehavior(JComponent)}, but
	 * that's impossible since the {@link BaselineResizeBehavior} class, which does not exist in Java 5, is involved.
	 *
	 * @return the baseline, or -2 if <code>getBaseline</code> could not be invoked on the delegate.
	 */
	@Override
