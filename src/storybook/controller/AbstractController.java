...
public abstract class AbstractController implements PropertyChangeListener {
	...
	protected synchronized void setModelProperty(String propertyName, Object newValue) {
		SbApp.trace(this.getClass().getSimpleName()
			+".setModelProperty("
			+ propertyName + "," + (newValue==null?"null":newValue.toString()) + ")");
		if (newValue != null) {
			if (newValue instanceof SbView) {
				SbApp.trace("newValue is SbView=" + ((SbView) newValue).getName());
			}
			for (AbstractModel model : attachedModels) {
				SbApp.trace("--> model="+model.getClass());
				Method method = null;
				Class<?>[] classes = null;
				try {
					if (newValue instanceof AbstractEntity) {
						...
						SbApp.trace("newValue != null > " + newValue.toString());
						classes = new Class[] { newValue.getClass() };
					}
					method = model.getClass().getMethod("set" + propertyName, classes);
					SbApp.trace("method : "+method.toGenericString());
					if (newValue != null) {
						SbApp.trace("invoke method with newValue");
						method.invoke(model, newValue);
					} else {
						SbApp.trace("invoke method without any value");
						method.invoke(model);
					}
					...
					if (e.getCause()!=null) {
						...
						String emethod=(method==null?"null":method.toString());
						String eclasses=(classes.getClass()==null?"null":classes[0].getClass().toString());
						System.err.println("*** : AbstractController.setModelProperty("+propertyName+","+newValue.toString()+")"
										+ "\n>>>msg=" + emsg
										+ "\n>>>method=" + emethod
										+ "\n>>>classes=" + eclasses);
					} else {
