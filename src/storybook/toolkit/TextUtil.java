...
public class TextUtil {
	...
	public static int countWords(String text) {
		SbApp.trace("TextUtil.countWords(text)");
		if (text==null || text.isEmpty()) return(0);
		int count = 0;
		String [] words=HtmlUtil.htmlToText(text).split(" ");
		count=words.length;
		for (String word:words) {
			if (word.length()==0 ) count--;
			if (word.length()==1) {
				if (".,;/§!%$£=+})°]@^\\_`|[({'#\"~&³*œ".contains(word)) count--;
			}
		}
		/*
		StringTokenizer stk = new StringTokenizer(HtmlUtil.htmlToText(text), " ");
