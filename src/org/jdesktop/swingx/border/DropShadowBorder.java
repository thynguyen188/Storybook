...
public class DropShadowBorder implements Border, Serializable {
    ...
    private Map<Position,BufferedImage> getImages(Graphics2D g2) {
        ...
        Map<Position,BufferedImage> images = CACHE.get(shadowSize + (shadowColor.hashCode() * .3) + (shadowOpacity * .12));
        if (images == null) {
...
//                buffer.setColor(new Color(0.0f, 0.0f, 0.0f, shadowOpacity));
            ...
            image.flush();
            CACHE.put(shadowSize + (shadowColor.hashCode() * .3) + (shadowOpacity * .12), images);
        }
        return images;
    }
