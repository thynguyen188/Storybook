...
/**
 * Creates user interface elements based on action ids and lists of action ids. All action ids must represent actions
 * managed by the ActionManager.
 * <p>
 * <h3>Action Lists</h3>
 * Use the createXXX(List) methods to construct containers of actions like menu bars, menus, popups and toolbars from
 * actions represented as action ids in a
 ...
 * <li>java.util.List: represents a submenu. See the note below which describes the configuration of menus.
 * </li>
 * </ul>
 * The order of elements in an action-list determines the arrangement of the ui components which are contructed from the
 * action-list.
 * <p>
 * For a menu or submenu, the first element in the action-list represents a menu and subsequent elements represent menu
 * items or separators (if null).
 * <p>
 * This class can be used as a general component factory which will construct components from Actions if the
 * <code>create&lt;comp&gt;(Action,...)</code> methods are used.
 *
 * @see ActionManager
 */
public class ActionContainerFactory {
	 ...
	 *
	 * @param manager use the actions managed with this manager for constructing ui componenents.
	 */
	...
	/**
	 * Gets the ActionManager instance. If the ActionManager has not been explicitly set then the default ActionManager
	...
	/**
	 * Constructs a toolbar from an action-list id. By convention, the identifier of the main toolbar should be
	 * "main-toolbar"
	 *
	...
	/**
	 * Creates and returns a menu from a List which represents actions, separators and sub-menus. The menu constructed
	 * will have the attributes from the first action in the List. Subsequent actions in the list represent menu items.
	 *
	 * @param actionIds an array of action ids used to construct the menu and menu items. the first element represents
	 * the action used for the menu,
	 ...
	 * will have the attributes from the first action in the List. Subsequent actions in the list represent menu items.
	 *
	 * @param list a list of action ids used to construct the menu and menu items. the first element represents the
	 * action used for the menu,
	 * @return the constructed JMenu or null
	 */
	...
	/**
	 * Creates a menu item based on the attributes of the action element. Will return a JMenuItem, JRadioButtonMenuItem
	 * or a JCheckBoxMenuItem depending on the context of the Action.
	 *
	...
	/**
	 * Creates a menu item based on the attributes of the action. Will return a JMenuItem, JRadioButtonMenuItem or a
	 * JCheckBoxMenuItem depending on the context of the Action.
	 *
	 ...
	 *
	 * The attributes of the action element registered with the ActionManger by the given id. Will return a JButton or a
	 * JToggleButton.
	 *
	...
	/**
	 * Creates a button based on the attributes of the action. If the container parameter is non-null then it will be
	 * used to uniquely identify the returned component within a ButtonGroup. If the action doesn't represent a grouped
	 * component then this value can be null.
	 *
	 * @param action an action used to create the button
	 * @param container the parent container to uniquely identify grouped components or null
	...
	/**
	 * method to configure a "selectable" button from the given AbstractActionExt. As there is some un-/wiring involved
	 * to support synch of the selected property between the action and the button, all config and unconfig (== setting
	 * a null action!) should be passed through this method.
	 * <p>
	 *
	 * It's up to the client to only pass in button's where selected and/or the group property makes sense.
	 *
	 * PENDING: the group properties are yet untested. PENDING: think about automated unconfig.
	 *
