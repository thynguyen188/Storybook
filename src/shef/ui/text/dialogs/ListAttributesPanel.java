...
public class ListAttributesPanel extends HTMLAttributeEditorPanel {
	...
	private static final String UL = I18N.getMsg("shef.listunordered"); //$NON-NLS-1$
	private static final String OL = I18N.getMsg("shef.listordered");     //$NON-NLS-1$
	...
	private void updateForType() {
		...
		if (typeCombo.getSelectedItem().equals(UL)) {
			for (String type : UL_TYPE_LABELS) {
				styleCombo.addItem(type);
			}
			startAtCB.setEnabled(false);
			startAtField.setEnabled(false);
		} else {
			for (String type : OL_TYPE_LABELS) {
				styleCombo.addItem(type);
			}
