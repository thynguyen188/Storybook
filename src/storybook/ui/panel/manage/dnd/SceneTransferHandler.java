...
public class SceneTransferHandler extends TransferHandler {
	...
	private boolean moveScene(long sourceSceneId, DTScenePanel destDtScene) {
		try {
			...
			session.close();
			for (Scene s : scenes) {
				if (s.getSceneno() != null && s.getSceneno() == -99) {
					// insert scene
	...
	private boolean swapScenes(long sourceSceneId, DTScenePanel destDtScene) {
		...
		Session session = model.beginTransaction();
		BookController ctrl = mainFrame.getBookController();
		Scene srcScene = (Scene) session.get(Scene.class, sourceSceneId);
		Scene destScene = destDtScene.getScene();
		session.close();
		Chapter srcChapter = srcScene.getChapter();
		int srcSceneno = srcScene.getSceneno();
		srcScene.setChapter(destScene.getChapter());
		srcScene.setSceneno(destScene.getSceneno());
		destScene.setChapter(srcChapter);
		destScene.setSceneno(srcSceneno);
		ctrl.updateScene(srcScene);
