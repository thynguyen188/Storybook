 ...
 *
 * <p>
 * A view can also contain a title bar that can be shown on either side of the view component. The title bar is made
 * visible by setting the visible property in the ViewTitleBarProperties in the ViewProperties for this view. The title
 * bar automatically inherits the view's title and icon but it's possible to specify a specific title and icon for the
 * title bar in the ViewTitleBarProperties in the ViewProperties for this view.
 * </p>
 *
...
public class View extends DockingWindow {
	...
	private HierarchyListener focusComponentListener = (HierarchyEvent e) -> {
	...
	private PropertyChangeListener listener = (Property property, Object valueContainer, Object oldValue, Object newValue) -> {
		fireTitleChanged();
	};

	private PropertyChangeListener titleBarPropertiesListener = (Property property, Object valueContainer, Object oldValue, Object newValue) -> {
	 ...
	 * </p>
	 * <p>
	 * The custom tab components will be shown after the window title when the window tab is highlighted. The components
	 * are shown in the same order as they appear in the list. The custom tab components container layout is rotated
	 * with the tab direction.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Returns a list containing the custom view title bar components. Changes to the list will be propagated to the
	 * title bar.
	 * </p>
	 * <p>
	 * The custom title bar components will be shown after the view title in the title bar but before the close,
	 * minimize and restore buttons. The components are shown in the same order as they appear in the list. The custom
	 * title bar components container layout is rotated with the title bar direction.
	 * </p>
	 * <p>
	 * <strong>Note:</strong> The components are only shon if the title bar is visible, see
	 * {@link ViewTitleBarProperties}.
	 * </p>
	 *
...
//      System.out.println("Focus: " + this + ", " + component.getClass() + " " + System.identityHashCode(component));
	...
	/**
	 * Restores focus to the last focused child component or, if no child component has had focus, the first focusable
		...
		if (changed || property == ViewTitleBarProperties.VISIBLE) {
			if (titleBarProperties.getVisible()) {
				if (titleBar == null) {
					...
					new DockingWindowDragSource(titleBar, (MouseEvent mouseEvent) -> getWindowProperties().getDragEnabled() ? startDrag(getRootWindow()) : null);
