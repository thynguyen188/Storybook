...
public class Scene extends AbstractEntity implements Comparable<Scene> {
    ...
    public void setSummary(String str) {
		if (str==null) this.summary=null;
		else  this.summary = str.replace("<html>","").replace("</html>","")
			.replace("<head>","").replace("</head>","")
			.replace("<body>","").replace("</body>","");
    }
    ...
    public Date getRelativeDate(Scene relativeScene) {
        ...
        if ((date == null)||(this.relativeDateDifference==null)) {
            return null;
        }
        return DateUtil.addDays(date, relativeDateDifference);
    }
	...
	public String toCsv(String quoteStart,String quoteEnd, String separator) {
		...
		b.append(quoteStart).append(getStatus().toString()).append(quoteEnd).append(separator);
		b.append(quoteStart).append(getInformative().toString()).append(quoteEnd).append(separator);
		b.append(quoteStart).append(getClean(getRelativeDateDifference())).append(quoteEnd).append(separator);
		...
		b.append(quoteStart).append(getOdf()).append(quoteEnd).append(separator);
		b.append(quoteStart);
		...
		b.append(quoteEnd).append(separator);
		b.append(quoteStart).append(getSummary()).append(quoteEnd).append(separator);
    ...
    public String toXml() {
		StringBuilder b=new StringBuilder();
		b.append(xmlTab(1)).append("<scene \n");
		b.append(xmlCommon());
		b.append(xmlAttribute("number", getClean(getSceneno())));
		b.append(xmlAttribute("sceneTs", getClean(getSceneTs())));
		b.append(xmlAttribute("status", getClean(getStatus())));
		b.append(xmlAttribute("narrator", getClean(getNarrator())));
		b.append(xmlAttribute("informative", getInformative().toString()));
		b.append(xmlTab(2)).append(">\n");
		b.append(xmlMeta(2,"title", getClean(getTitle())));
		b.append(xmlMeta(2,"chapter", getClean(getChapter().getTitle())));
		b.append(xmlMeta(2,"strand", getClean(getStrand().getName())));
		b.append(xmlTab(2)).append("<relativeto \n");
		b.append(xmlAttribute("difference",getClean(getRelativeDateDifference())));
		b.append(xmlAttribute("scene",getClean(getRelativeSceneId())));
		b.append(xmlTab(2)).append("/>\n");
		b.append(xmlMeta(2,"odf",getOdf()));
		if (persons.size()>0) for (Person p:persons) {
			b.append(xmlMeta(2,"persons",p.getFullName()));
		}
		if (items!=null) for (Item p:items) {
			b.append(xmlMeta(2,"items",p.getName()));
		}
		if (locations!=null) for (Location p:locations) {
			b.append(xmlMeta(2,"locations",p.getName()));
		}
		if (strands!=null) for (Strand p:strands) {
			b.append(xmlMeta(2,"strands",p.getName()));
		}
		b.append(xmlMeta(2,"summary",getSummary()));
	...
	public static Scene fromXml(Node node) {
		...
		p.setTitle(getXmlText(node,"title"));
		p.setSceneno(getXmlInteger(node,"number"));
		String ts=getXmlString(node,"sceneTs");
		if (!ts.isEmpty()) {
			p.setSceneTs(Timestamp.valueOf(ts));
		}
		p.setStatus(getXmlInteger(node,"status"));
		p.setOdf(getXmlText(node,"odf"));
		p.setSummary(getXmlText(node,"summary"));
