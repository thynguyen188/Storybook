...
import org.apache.commons.text.time.FastDateFormat;
...
public class SwingUtil {
	...
	public static void setLookAndFeel() {
		try {
			...
			LookAndFeel laf = LookAndFeel.valueOf(SbApp.getInstance().preferences.getString(SbPref.Key.LAF, LookAndFeel.cross.name()));
			setLookAndFeel(laf);
		} catch (Exception e) {
...
//				break;
			...
			SbApp.getInstance().preferences.setString(SbPref.Key.LAF, lookAndFeel.name());
...
*/
	...
	public static JButton createButton(String text, String icon, String tips) {
		...
		if (!text.isEmpty()) bt.setText(I18N.getMsg(text));
		if (!icon.isEmpty()) bt.setIcon(new ImageIcon(SbApp.class.getResource("/storybook/resources/icons/"+icon+".png")));
		if (!tips.isEmpty()) bt.setToolTipText(I18N.getMsg(tips));
	...
	public static Font getSmalFont(int size) {
	...
	public static void addCopyPasteToPopupMenu(JPopupMenu menu, JComponent comp) {
		...
		miCopy.setText(I18N.getMsg("copy"));
		...
		miPaste.setText(I18N.getMsg("paste"));
	...
	public static void setNotes(JPanel panel, Object object) {
		String texte = null;
		if ((object instanceof Strand)&&(((Strand)object).hasNotes())) {
			texte = (((Strand) object).getNotes());
		} else if ((object instanceof Person)&&(((Person)object).hasNotes())) {
			texte = (((Person) object).getNotes());
		} else if ((object instanceof Part)&&(((Part)object).hasNotes())) {
			texte = (((Part) object).getNotes());
		}  else if ((object instanceof Chapter)&&(((Chapter)object).hasNotes())) {
			texte = (((Chapter) object).getNotes());
		} else if ((object instanceof Scene)&&(((Scene)object).hasNotes())) {
			texte = (((Scene) object).getNotes());
		} else if ((object instanceof Item)&&(((Item)object).hasNotes())) {
			texte = (((Item) object).getNotes());
		}  else if ((object instanceof Location)&&(((Location)object).hasNotes())) {
			texte = (((Location) object).getNotes());
		}  else if ((object instanceof Tag)&&(((Part)object).hasNotes())) {
			texte = (((Tag) object).getNotes());
		} 
		if (!HtmlUtil.htmlToText(texte).equals("")) {
			panel.setToolTipText("<html>" + texte + "</html>");
		} else {
	...
	public static void saveDlgPosition(JDialog dlg, AbstractEntity e) {
		...
		String pos=formX+","+formY+","+formH+","+formW;
		String keyName="Editor_"+e.getClass().getName().replace("storybook.model.hbn.entity.", "");
		SbPref pref=SbApp.getInstance().getPref();
		if (!pos.equals(pref.getString(keyName, ""))) pref.setString(keyName, pos);
	}
	...
	public static Dimension getDlgPosition(JDialog dlg, AbstractEntity e) {
		Dimension d=null;
		SbPref pref=SbApp.getInstance().getPref();
		String keyName="Editor_"+e.getClass().getName().replace("storybook.model.hbn.entity.", "");
		String x=pref.getString(keyName, "");
		if (x.isEmpty()) return(d);
		String z[]=x.split(",");
		d=new Dimension();
	...
	public static Dimension getDlgPosition(JFrame dlg, AbstractEntity e) {
		Dimension d=null;
		SbPref pref=SbApp.getInstance().getPref();
		String keyName="Editor_"+e.getClass().getName().replace("storybook.model.hbn.entity.", "");
		String x=pref.getString(keyName, "");
	...
	public static Dimension getDlgSize(JDialog dlg, AbstractEntity e) {
		Dimension d=null;
		SbPref pref=SbApp.getInstance().getPref();
		String keyName="Editor_"+e.getClass().getName().replace("storybook.model.hbn.entity.", "");
		String x=pref.getString(keyName, "");
	...
	public static Dimension getDlgSize(JFrame dlg, AbstractEntity e) {
		Dimension d=null;
		SbPref pref=SbApp.getInstance().getPref();
		String keyName="Editor_"+e.getClass().getName().replace("storybook.model.hbn.entity.", "");
		String x=pref.getString(keyName, "");
	...
	public static String setDefaultFont(MainFrame mainFrame, JTextPane ta) {
		Font font=ta.getFont();
		String r="style=\""
			+ "font-family: " +  mainFrame.getPref().getString(SbPref.Key.DEFAULT_FONT_NAME, "Serif") + "; "
			+ "font-size: " + mainFrame.getPref().getInteger(SbPref.Key.DEFAULT_FONT_SIZE, font.getSize()) + "pt;"
			+ "\"";
		return(r);
	}
