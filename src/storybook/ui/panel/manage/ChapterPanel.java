...
public class ChapterPanel extends AbstractPanel implements MouseListener, IRefreshable {
	...
	public void init() {
		...
		try {
			Internal internal = BookUtil.get(mainFrame, BookKey.MANAGE_ZOOM, SbConstants.DEFAULT_MANAGEZOOM);
			setZoomedSize(internal.getIntegerValue());
		} catch (Exception e) {
			System.err.println(Arrays.toString(e.getStackTrace()));
			setZoomedSize(SbConstants.DEFAULT_MANAGEZOOM);
		}
	}
	...
	public void initUi() {
		...
		if (chapter == null) {
			...
			buf.append(" ");
			buf.append(chapter.getTitle((prefWidth/16)*2));
			lbChapter.setText(buf.toString());
			lbChapter.setLineWrap(true);
			lbChapter.setWrapStyleWord(true);
			lbChapter.setToolTipText(chapter.getTitle());
			SwingUtil.setForcedSize(lbChapter,new Dimension(prefWidth, 32));
			add(lbChapter);
			this.setToolTipText(chapter.getTitle());
		}
	...
	public List<DTScenePanel> getDTScenePanels() {
		List<DTScenePanel> list = new ArrayList<>();
