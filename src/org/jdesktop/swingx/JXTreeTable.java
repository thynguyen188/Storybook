...
/**
 * <p>
 * <code>JXTreeTable</code> is a specialized {@link javax.swing.JTable table} consisting of a single column in which to
 * display hierarchical data, and any number of other columns in which to display regular data. The interface for the
 * data model used by a <code>JXTreeTable</code> is {@link org.jdesktop.swingx.treetable.TreeTableModel}. It extends the
 * {@link javax.swing.tree.TreeModel} interface to allow access to cell data by column indices within each node of the
 * tree hierarchy.</p>
 *
 * <p>
 * The most straightforward way create and use a <code>JXTreeTable</code>, is to first create a suitable data model for
 * it, and pass that to a <code>JXTreeTable</code> constructor, as shown below:
 * <pre>
 *  TreeTableModel  treeTableModel = new FileSystemModel(); // any TreeTableModel
 *  JXTreeTable     treeTable = new JXTreeTable(treeTableModel);
 *  JScrollPane     scrollpane = new JScrollPane(treeTable);
 * </pre> See {@link javax.swing.JTable} for an explanation of why putting the treetable inside a scroll pane is
 * necessary.</p>
 *
 * <p>
 * A single treetable model instance may be shared among more than one <code>JXTreeTable</code> instances. To access the
 * treetable model, always call {@link #getTreeTableModel() getTreeTableModel} and
 * {@link #setTreeTableModel(org.jdesktop.swingx.treetable.TreeTableModel) setTreeTableModel}. <code>JXTreeTable</code>
 * wraps the supplied treetable model inside a private adapter class to adapt it to a
 * {@link javax.swing.table.TableModel}. Although the model adapter is accessible through the
 * {@link #getModel() getModel} method, you should avoid accessing and manipulating it in any way. In particular, each
 * model adapter instance is tightly bound to a single table instance, and any attempt to share it with another table
 * (for example, by calling {@link #setModel(javax.swing.table.TableModel) setModel}) will throw an
...
public class JXTreeTable extends JXTable {
	...
	/**
	 * Renderer used to render cells within the {@link #isHierarchical(int) hierarchical} column. renderer extends
	 * JXTree and implements TableCellRenderer
	 */
	private TreeTableCellRenderer renderer;

	/**
	 * Editor used to edit cells within the {@link #isHierarchical(int) hierarchical} column.
	 */
	...
	/**
	 * Constructs a JXTreeTable using a {@link org.jdesktop.swingx.treetable.DefaultTreeTableModel}.
	 */
	public JXTreeTable() {
	...
	/**
	 * Constructs a JXTreeTable using the specified {@link org.jdesktop.swingx.treetable.TreeTableModel}.
	 *
...
//        setIntercellSpacing(spacing); // for both row margin and column margin
	 ...
	 *
	 * TreeTable is not sortable because there is no equivalent to RowSorter (which is targeted to linear structures)
	...
	/**
	 * Overriden to invoke repaint for the particular location if the column contains the tree. This is done as the tree
	 * editor does not fill the bounds of the cell, we need the renderer to paint the tree in the background, and then
	 * draw the editor over it. You should not need to call this method directly.
	 * <p>
	 *
	...
	/**
	 * Hacking around various issues. Subclass and let it return your favourite. The current default is
	 * TreeTableHackerExt5 (latest evolution to work around #1230), the old long-standing default was
	 * TreeTableHackerExt3. If you experience problems with the latest, please let us know.
	 *
	 * @return
	 */
...
//        return new TreeTableHackerExt4();
	...
	/**
	 * This class extends TreeTableHackerExt instead of TreeTableHackerExt3 so as to serve as a clue that it is a
	 * complete overhaul and looking in TreeTableHackerExt2 and TreeTableHackerExt3 for methods to change the behavior
	 * will do you no good.
	 * <p>
	 * The methods previously used are abandoned as they would be misnomers to the behavior as implemented in this
	 * class.
	 * <p>
	 * Changes:
	 * <ol>
	 * <li>
	 * According to TreeTableHackerExt3, clickCounts > 1 are not sent to the JTree so that double clicks will start
	 * edits (Issue #474). Well, mouse events are only sent to the JTree if they occur within the tree handle space - so
	 * that is not the behavior desired. Double clicks on the text/margin opposite the tree handle already started edits
	 * without that modification (I checked). The only thing that modification does is introduce bugs when one actually
	 * double clicks on a tree handle... so that idea was abandoned.</li>
	 * <li>
	 * There is no longer any discrimination between events that cause an expansion/collapse. Since the event location
	 * is check to see if it is in the tree handle margin area, this doesn't seem necessary. Plus it is more user
	 * friendly: if someone missed the tree handle by 1 pixel, then it caused a selection change instead of a node
	 * expansion/ collapse.</li>
	 * <li>
	 * The consumption of events are handled within this class itself because the behavior associated with the way that
	 * <code>processMoueEvent(MouseEvent)</code> consumed events was incompatible with the way this class does things.
	 * As a consequence, <code>hitHandleDetectionFromProcessMouse(MouseEvent)</code> always returns false so that
	 * <code>processMoueEvent(MouseEvent)</code> will not doing anything other than call its super method.</li>
	 * <li>
	 * All events of type MOUSE_PRESSED, MOUSE_RELEASED, and MOUSE_CLICKED, but excluding when
	 * <code>isPopupTrigger()</code> returns true, are sent to the JTree. This has the added benefit of not having to
	 * piggy back a mouse released event as we can just use the real mouse released event. This keeps the look and feel
	 * consistent for the user of UI's that expand/collapse nodes on the release of the mouse.</li>
	 * <li>
	 * The previous implementations have a spiel about avoiding events with modifiers because the UI might try to change
	 * the selection. Well that didn't occur in any of the look and feels I tested. Perhaps that was the case if events
	 * that landed within the content area of a node were sent to the JTree. If that behavior is actually necessary,
	 * then it can be added to the <code>isTreeHandleEventType(MouseEvent)</code> method. This implementation sends all
	 * events regardless of the modifiers.</li>
	 * <li>
	 * This implementation toggles the processing of mouse motion events. When events are sent to the tree, it is turned
	 * off and turned back on when an event is not sent to the tree. This fixes selection changes that occur when one
	 * drags the mouse after pressing on a tree handle.</li>
	 * </ol>
	 *
	...
	public class TreeTableHackerExt4 extends TreeTableHackerExt {
		...
		/**
		 * Filter to find mouse events that are candidates for node expansion/ collapse. MOUSE_PRESSED and
		 * MOUSE_RELEASED are used by default UIs. MOUSE_CLICKED is included as it may be used by a custom UI.
		 *
		...
		/**
		 * This method checks if the location of the event is in the tree handle margin and translates the coordinates
		 * for the JTree.
		 *
		 * @param e the currently dispatching mouse event
		 * @return the mouse event to dispatch to the JTree or null if nothing should be dispatched
		 */
		...
		/**
		 *
		 * @return this method always returns false, so that processMouseEvent always just simply calls its super method
		 */
		@Override
	...
	public class TreeTableHackerExt5 extends TreeTableHackerExt4 {
		...
		/**
		 * If a negative number is returned, then all events that occur in the leading margin will be forwarded to the
		 * tree and consumed.
		 *
	...
	/**
	 * Temporary class to have all the hacking at one place. Naturally, it will change a lot. The base class has the
	 * "stable" behaviour as of around jun2006 (before starting the fix for 332-swingx).
	 * <p>
	 *
	 * specifically:
	 *
	...
	public class TreeTableHacker {
		...
		/**
		 * Decision whether the handle hit detection should be done in processMouseEvent or editCellAt. Here: returns
		 * false.
		 *
		 * @return true for handle hit detection in processMouse, false for editCellAt.
		 */
		...
		/**
		 * Entry point for hit handle detection called from editCellAt, does nothing if isHitDetectionFromProcessMouse
		 * is true;
		 *
		...
		/**
		 * Entry point for hit handle detection called from processMouse. Does nothing if isHitDetectionFromProcessMouse
		 * is false.
		 *
		 * @return true if the mouseEvent triggered an expand/collapse in the renderer, false otherwise.
		 *
		 * @see #isHitDetectionFromProcessMouse()
		 */
		public boolean hitHandleDetectionFromProcessMouse(MouseEvent e) {
		 ...
		 * <p>
		 * This is part of fix for #730-swingx - editingStopped not always called. The other part is to call this from
		 * the renderer before expansion related state has changed.
		 * <p>
		 *
		 * Was: any editing is always cancelled.
		 * <p>
		 * This is a rude fix to #120-jdnc: data corruption on collapse/expand if editing. This is called from the
		 * renderer after expansion related state has changed.
		 *
		 */
		 ...
		 * <ol>
		 * <li> editCellAt: original, stable but buggy (#332, #222) the table's own selection had been changed due to
		 * the click before even entering into editCellAt so all tree selection state is lost.
		 *
		 * <li> processMouseEvent: the idea is to catch the mouseEvent, check if it triggered an expanded/collapsed,
		 * consume and return if so or pass to super if not.
		 * </ol>
		 *
		 * <p>
		 ...
		 *
		 * @return this methods evaluation as to whether the event triggered a expand/collaps
		 */
	...
	/**
	 *
	 * Note: currently this class looks a bit funny (only overriding the hit decision method). That's because the
	 * "experimental" code as of the last round moved to stable. But I expect that there's more to come, so I leave it
	 * here.
	 *
	 * <ol>
	 ...
	 * <p>
	 *
	 * Note: with 1.6 the expansion control was broken even with the "normal extended" TreeTableHackerExt. When fixing
	 * that (renderer must have correct width for BasicTreeUI since 1.6) took a look into why this didn't work and made
	 * it work. So, now this is bidi-compliant.
	 *
	...
	public class TreeTableHackerExt2 extends TreeTableHackerExt {
		...
		protected boolean expandOrCollapseNode(int column, EventObject e) {
			...
			if (me.getModifiers() == 0
				...
				if (treeRow < 0) {
					...
					if (bounds == null) {
						row = -1;
					} else // check if the mouse location is "leading"
					...
					} else if ((bounds.y + bounds.height < treeMousePoint.y)
		...
		/**
		 * This is a patch provided for Issue #980-swingx which should improve the bidi-compliance. Still doesn't work
		 * in our visual tests...<p>
		 *
		 * Problem was not in the translation to renderer coordinate system, it was in the method itself: the check
		 * whether we are "beyond" the cell content box is bidi-dependent. Plus (since 1.6), width of renderer must be >
		 * 0.
		 *
		 *
		 * @param column the column index under the event, if any.
	...
	/**
	 * A more (or less, depending in pov :-) aggressiv hacker. Compared to super, it dispatches less events to address
	 * open issues.<p>
	 *
	 * Issue #474-swingx: double click should start edit (not expand/collapse) changed mightBeExpansionTrigger to filter
	 * out clickCounts > 1
	 * <p>
	 * Issue #875-swingx: cell selection mode changed the dispatch to do so only if mouse event outside content box and
	 * leading
	 * <p>
	 * Issue #1169-swingx: remove 1.5 dnd hack removed the additional dispatch here and changed in the implementation of
	 * hackAroundDragEnabled to no longer look for the system property (it's useless even if set)
	 *
	 * @author tiberiu@dev.java.net
	 */
	...
	/**
	 * Decides whether we want to apply the hack for #168-jdnc. here: returns true if dragEnabled() and a client
	 * property with key DRAG_HACK_FLAG_KEY has a value of boolean true.<p>
	 *
	 * Note: this is updated for 1.6, as the intermediate system property for enabled drag support is useless now (it's
	 * the default)
	 *
	...
	/**
	 * Overridden to provide a workaround for BasicTableUI anomaly. Make sure the UI never tries to resize the editor.
	 * The UI currently uses different techniques to paint the renderers and editors. So, overriding setBounds() is not
	 * the right thing to do for an editor. Returning -1 for the editing row in this case, ensures the editor is never
	 * painted.
	 *
	 * {@inheritDoc}
	 */
	...
	/**
	 * Sets the data model for this JXTreeTable to the specified {@link org.jdesktop.swingx.treetable.TreeTableModel}.
	 * The same data model may be shared by any number of JXTreeTable instances.
	 *
...
//        setRootVisible(rootVisible);
	...
	/**
	 * <p>
	 * Overrides superclass version to make sure that the specified {@link javax.swing.table.TableModel} is compatible
	 * with JXTreeTable before invoking the inherited version.</p>
	 *
	 * <p>
	 * Because JXTreeTable internally adapts an {@link org.jdesktop.swingx.treetable.TreeTableModel} to make it a
	 * compatible TableModel, <b>this method should never be called directly</b>. Use
	 * {@link #setTreeTableModel(org.jdesktop.swingx.treetable.TreeTableModel) setTreeTableModel} instead.</p>
	 *
	 * <p>
	 * While it is possible to obtain a reference to this adapted version of the TableModel by calling
	 * {@link javax.swing.JTable#getModel()}, any attempt to call setModel() with that adapter will fail because the
	 * adapter might have been bound to a different JXTreeTable instance. If you want to extract the underlying
	 * TreeTableModel, which, by the way,
	 * <em>can</em> be shared, use {@link #getTreeTableModel() getTreeTableModel} instead</p>.
	 *
	 * @param tableModel must be a TreeTableModelAdapter
	 * @throws IllegalArgumentException if the specified tableModel is not an instance of TreeTableModelAdapter
	 */
	@Override
	...
	/**
	 * Forwards treeRowHeight to table. This is for completeness only: the rendering tree is under our total control, so
	 * we don't expect any external call to tree.setRowHeight.
	 *
	...
	/**
	 * <p>
	 * Overridden to ensure that private renderer state is kept in sync with the state of the component. Calls the
	 * inherited version after performing the necessary synchronization. If you override this method, make sure you call
	 * this version from your version of this method.</p>
	 *
	 * <p>
	 * This version maps the selection mode used by the renderer to match the selection mode specified for the table.
	 * Specifically, the modes are mapped as follows:
	 * <pre>
	 ...
	 * <p>
	 *
	 * Overridden to decorate the tree's renderer after calling super. At that point, it is only the tree itself that
	 * has been decorated.
	 *
	...
	/**
	 * Performs configuration of the tree's renderer if the adapter's column is the hierarchical column, does nothing
	 * otherwise.
	 * <p>
	 *
	 * Note: this is legacy glue if the treeCellRenderer is of type DefaultTreeCellRenderer. In that case the renderer's
	 * background/foreground/Non/Selection colors are set to the tree's background/foreground depending on the adapter's
	 * selection state. Does nothing if the treeCellRenderer is backed by a ComponentProvider.
	 *
	 ...
	 *
	 * @param overwrite a boolean to indicate if the per-tree Icons should be copied to the new renderer on
	 * setTreeCellRenderer.
	 *
	 ...
	 *
	 * @return true if a TreeCellRenderer's icons will be overwritten with the tree's Icons, false if the renderer's
	 * icons will be unchanged.
	 *
	...
	/**
	 * Makes sure all the path components in path are expanded (except for the last path component) and scrolls so that
	 * the node identified by the path is displayed. Only works when this <code>JTree</code> is contained in a
	 * <code>JScrollPane</code>.
	 *
	 * (doc copied from JTree)
	 *
	 * PENDING: JW - where exactly do we want to scroll? Here: the scroll is in vertical direction only. Might need to
...
//        scrollRowToVisible(row);
	...
	/**
	 * Expands the specified row in the treetable. If the specified row index is not valid, this method will have no
	 * effect.
	 */
	...
	/**
	 * Returns true if the value identified by path is currently viewable, which means it is either the root or all of
	 * its parents are expanded. Otherwise, this method returns false.
	 *
	 * @return true, if the value identified by path is currently viewable; false, otherwise
	 */
	 ...
	 *
	 * @param path path
	 * @return true, if the value identified by path is currently expanded; false, otherwise
	 */
	...
	/**
	 * Returns true if the node at the specified display row is currently expanded. Otherwise, this method returns
	 * false.
	 *
	 * @param row row
	 * @return true, if the node at the specified display row is currently expanded. false, otherwise
	 */
	...
	/**
	 * Returns true if the node identified by path is currently collapsed, this will return false if any of the values
	 * in path are currently not being displayed.
	 *
	 * @param path path
	 * @return true, if the value identified by path is currently collapsed; false, otherwise
	 */
	 ...
	 *
	 * @param row row
	 * @return true, if the node at the specified display row is currently collapsed. false, otherwise
	 */
	...
	/**
	 * Returns an <code>Enumeration</code> of the descendants of the path <code>parent</code> that are currently
	 * expanded. If <code>parent</code> is not currently expanded, this will return <code>null</code>. If you
	 * expand/collapse nodes while iterating over the returned <code>Enumeration</code> this may not return all the
	 * expanded paths, or may return paths that are no longer expanded.
	 *
	 * @param parent the path which is to be examined
	 * @return an <code>Enumeration</code> of the descendents of <code>parent</code>, or <code>null</code> if
...
//------------------------------ exposed Tree properties
	...
	/**
	 * Sets the value of the <code>scrollsOnExpand</code> property for the tree part. This property specifies whether
	 * the expanded paths should be scrolled into view. In a look and feel in which a tree might not need to scroll when
	 * expanded, this property may be ignored.
	 *
	 * @param scroll true, if expanded paths should be scrolled into view; false, otherwise
	 */
	...
	/**
	 * Sets the value of the <code>showsRootHandles</code> property for the tree part. This property specifies whether
	 * the node handles should be displayed. If handles are not supported by a particular look and feel, this property
	 * may be ignored.
	 *
	...
	/**
	 * Sets the value of the <code>expandsSelectedPaths</code> property for the tree part. This property specifies
	 * whether the selected paths should be expanded.
	 *
	 ...
	 * <p>
	 *
	 * <strong>NOTE</strong>: this method is exposed for completeness - currently it's not recommended to use a large
	 * model because there are some issues (not yet fully understood), namely issue #25-swingx, and probably
	 * #270-swingx.
	 *
...
//------------------------------ exposed tree listeners
	 ...
	 *
	 * @param tel a TreeExpansionListener that will be notified when a tree node is expanded or collapsed
	 */
	 ...
	 *
	 * @param tsl a TreeSelectionListener that will be notified when a tree node is selected or deselected
	 */
	 ...
	 *
	 * @param tel a TreeWillExpandListener that will be notified when a tree node will be expanded or collapsed
	 */
	...
	/**
	 * Overriden to invoke supers implementation, and then, if the receiver is editing a Tree column, the editors bounds
	 * is reset. The reason we have to do this is because JTable doesn't think the table is being edited, as
	 * <code>getEditingRow</code> returns -1, and therefore doesn't automaticly resize the editor for us.
	 */
	@Override
	 ...
	 * @throws IllegalArgumentException if the column is less than 0 or greater than or equal to the column count
	 */
	 ...
	 *
	 * @return the index of the hierarchical column, -1 if there is no hierarchical column
	 *
	 */
	...
	/**
	 * Updates Ui of renderer/editor for the hierarchical column. Need to do so manually, as not accessible by the
	 * default lookup.
	 */
	 ...
	 * <p>
	 *
	 * PENDING JW: revisit once we switch to really using a table renderer. As is, it's a quick fix for #821-swingx:
	 * string rep for hierarchical column incorrect.
	 */
	@Override
	...
	protected static class TreeTableModelAdapter extends AbstractTableModel {
		...
		/**
		 * Maintains a TreeTableModel and a JTree as purely implementation details. Developers can plug in any type of
		 * custom TreeTableModel through a JXTreeTable constructor or through setTreeTableModel().
		 *
		 * @param model Underlying data model for the JXTreeTable that will ultimately be bound to this
		 * TreeTableModelAdapter
		 * @param tree TreeTableCellRenderer instantiated with the same model as specified by the model parameter of
		 * this constructor
...
//            treeTable.getTreeTableHacker().setExpansionChangedFlag();
		 ...
		 * For use by {@link org.jdesktop.swingx.JXTreeTable#setModel(javax.swing.table.TableModel)}.
		 *
...
//                        LOG.info("got tree event: removed " + e);
		...
		/**
		 * Decides if the given treeModel structureChanged should trigger a table structureChanged. Returns true if the
		 * source path is the root or null, false otherwise.<p>
		 *
		 * PENDING: need to refine? "Marker" in Event-Object?
		 *
		 * @param e the TreeModelEvent received in the treeModelListener's treeStructureChanged
		 * @return a boolean indicating whether the given TreeModelEvent should trigger a structureChanged.
		 */
		...
		/**
		 * Invokes fireTableDataChanged after all the pending events have been processed. SwingUtilities.invokeLater is
		 * used to handle this.
		 */
		...
		/**
		 * Invokes fireTableDataChanged after all the pending events have been processed. SwingUtilities.invokeLater is
		 * used to handle this. Allowed event types: 1 for insert, 2 for delete
		 */
...
//                                                + max);
		...
		/**
		 * This is used for updated only. PENDING: not necessary to delay? Updates are never structural changes which
		 * are the critical.
		 *
		 * @param tme
		 */
...
//                            LOG.info("Updated: parentPath/min/max" + path + "/" + min + "/" + max);
	...
	static class TreeTableCellRenderer extends JXTree implements
		TableCellRenderer // need to implement RolloverRenderer
	...
	//      ,  RolloverRenderer 
	{
		 ...
		 * <p>
		 *
		 * Overridden to hack around #766-swingx: cursor flickering in DnD when dragging over tree column. This is a
		 * core bug (#6700748) related to painting the rendering component on a CellRendererPane. A trick around is to
		 * let this return false.
		 * <p>
		 *
		 * This implementation applies the trick, that is returns false always. The hack can be disabled by setting the
		 * treeTable's client property DROP_HACK_FLAG_KEY to Boolean.FALSE.
		 *
		 */
		@Override
		 ...
		 *
		 * The problem is that - due to much tricksery when rendering the tree - the given coordinates are rather
		 * useless. As a consequence, super maps to wrong coordinates. This takes over completely.
		 *
		 * PENDING: bidi?
		 *
		...
		private String getToolTipText(MouseEvent event, int row, int column) {
			...
			Component rComponent = renderer.getTreeCellRendererComponent(this, lastPath, isRowSelected(row),
...
//                p.translate(-pathBounds.x, -pathBounds.y);
		 ...
		 * <p>
		 *
		 * Overridden to not automatically de/register itself from/to the ToolTipManager. As rendering component it is
		 * not considered to be active in any way, so the manager must not listen.
		 */
		@Override
		...
		/**
		 * Immutably binds this TreeTableModelAdapter to the specified JXTreeTable. For internal use by JXTreeTable
		 * only.
		 *
...
//                bindRollover();
		...
		/**
		 * Install rollover support. Not used - still has issues. - not bidi-compliant - no coordinate transformation
		 * for hierarchical column != 0 - method name clash enabled - keyboard triggered click unreliable (triggers the
		 * treetable) ...
		 */
		@SuppressWarnings("unused")
		 ...
		 *
		 * This implementation - Synchs the tree's rolloverEnabled - maps rollover cell from the table to the cell
		 ...
		 * <p>
		 *
		 * Overridden to produce clicked client props only. The rollover are produced by a propertyChangeListener to the
		 * table's corresponding prop.
		 *
		 */
		@Override
		protected RolloverProducer createRolloverProducer() {
			return new RolloverProducer() {
				...
				protected void updateRolloverPoint(JComponent component,
					...
					if (bounds == null) {
						row = -1;
					} else if ((bounds.y + bounds.height < mousePoint.y)
...
//               new IllegalArgumentException("the rendering tree must have a fixed rowHeight > 0");
		...
		/**
		 * This is overridden to set the location to (0, 0) and set the dimension to exactly fill the bounds of the
		 * hierarchical column.<p>
		 */
		@Override
...
//                }
	...
	/**
	 * Returns the adapter that knows how to access the component data model. The component data adapter is used by
	 * filters, sorters, and highlighters.
	 *
	...
	protected static class TreeTableDataAdapter extends JXTable.TableAdapter {
		...
		/**
		 * Constructs a <code>TreeTableDataAdapter</code> for the specified target component.
		 *
		...
		/**
		 *
		 * @return true if the cell identified by this adapter displays hierarchical nodes; false otherwise
		 */
		@Override
		 ...
		 * <p>
		 *
		 * Overridden to fix #821-swingx: string rep of hierarchical column incorrect. In this case we must delegate to
		 * the tree directly (via treetable.getHierarchicalString).
		 *
